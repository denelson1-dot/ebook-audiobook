...
def _logger() -> logging.Logger:
    return open_logger(log_path())


def open_logger(path: Path, max_bytes: int | None = None,
                backup_count: int | None = None) -> logging.Logger:
    """One rotating handler per log file. Also used by :mod:`debuglog`, so
    :func:`_close_handlers` releases both before anything is deleted.
    ...
    handler = logging.handlers.RotatingFileHandler(
        path, maxBytes=MAX_BYTES if max_bytes is None else max_bytes,
        backupCount=BACKUP_COUNT if backup_count is None else backup_count, encoding="utf-8",
    )
...
def issue_report(limit: int = 3) -> str:
    ...
    try:  # the performance log, when someone turned it on
        from .debuglog import report_lines
    ...
    if not found:
        ...
        lines += perf
        lines.append(f"_Report generated by ebook-audiobook {__version__}._")
        return "\n".join(lines)
